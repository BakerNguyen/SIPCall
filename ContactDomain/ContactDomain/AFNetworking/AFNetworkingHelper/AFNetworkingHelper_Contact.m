...
@implementation AFNetworkingHelper_Contact
                                      ...
                                      fail:(id)failBlock
{
    ...
    AFHTTPRequestOperation_Contact *op = [[AFHTTPRequestOperation_Contact alloc] initWithRequest:req];
    op.responseSerializer = [AFPropertyListResponseSerializer_Contact serializer];
...
+ (void)requestJSONAFHTTPRequestKindOperation:(NSString *)urlString
                                         ...
                                         fail:(id)failBlock
{
    ...
    if (isPOST)
    {
        [manager POST:urlString parameters:parameters success:successBlock failure:failBlock];
    }
    else
    {
        [manager GET:urlString parameters:parameters success:successBlock failure:failBlock];
    }
...

{
    ...
    AFHTTPRequestOperation_Contact *op = [[AFHTTPRequestOperation_Contact alloc] initWithRequest:req];
    op.responseSerializer = [AFJSONResponseSerializer_Contact serializer];
    op.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/html"];
    [op setCompletionBlockWithSuccess:successBlock failure:failBlock];
...
outputStreamToFileAtPath:FilePath
{
    NSURLRequest *req = [NSURLRequest requestWithURL:fileURL
                                         cachePolicy:NSURLRequestReloadIgnoringLocalCacheData
                                     timeoutInterval:timeoutInterval];
    AFHTTPRequestOperation_Contact *operation = [[AFHTTPRequestOperation_Contact alloc] initWithRequest:req];
                                    ...
                                    fail:(id)failBlock
{
    ...
    AFHTTPRequestOperation_Contact *op = [[AFHTTPRequestOperation_Contact alloc] initWithRequest:req];
    op.responseSerializer = [AFXMLParserResponseSerializer_Contact serializer];
    [op setCompletionBlockWithSuccess:successBlock failure:failBlock];
                         ...
                         method:(NSString *)method
{
    ...
    {
        if (![operation isKindOfClass:[AFHTTPRequestOperation_Contact class]])
        {
            continue;
        }

        BOOL hasMatchingMethod = !method || [method isEqualToString:[[(AFHTTPRequestOperation_Contact *)operation request] HTTPMethod]];
        BOOL hasMatchingURL = [[[[(AFHTTPRequestOperation_Contact *)operation request] URL] absoluteString] isEqualToString:URLStringToMatched];
...
+ (void)getShareNetworkStatus:(id)success
{
    ...
    [[AFNetworkReachabilityManager_Contact sharedManager] startMonitoring];
    [[AFNetworkReachabilityManager_Contact sharedManager] setReachabilityStatusChangeBlock:success];
}
                             ...
                             fail:(id)failBlock
{
    ...
    AFHTTPRequestOperation_Contact *op = [[AFHTTPRequestOperation_Contact alloc] initWithRequest:request];
                                ...
                                fail:(id)failBlock
{
    ...
    for (NSURL *fileURL in filesToUpload)
    {
        ...
        AFHTTPRequestOperation_Contact *operation = [[AFHTTPRequestOperation_Contact alloc] initWithRequest:request];

        [mutableOperations addObject:operation];
    }

    NSArray *operations = [AFURLConnectionOperation_Contact batchOfRequestOperations:batch
