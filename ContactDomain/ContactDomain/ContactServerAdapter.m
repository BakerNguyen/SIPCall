...
@implementation ContactServerAdapter
...
#define CENTRAL_PORT @"443"
#define CENTRAL_ENCRYPTION @"1"

#define ERROR_CODE_TIME_OUT -1001

#define API_CHECK_SERVER_MAINTERNACE @"ZP9jF8NOyd"
#define API_CHECK_SERVER_MAINTERNACE_VERSION @"v1"
...
+(ContactServerAdapter *)share{
    ...
    static ContactServerAdapter * share;
    dispatch_once(&once, ^{
        share = [self new];
        // Configure CocoaLumberjack
        [DDLog addLogger:[DDASLLogger_Contact sharedInstance]];
        [DDLog addLogger:[DDTTYLogger_Contact sharedInstance]];
        setenv("XcodeColors", "YES", 0);
        [[DDTTYLogger_Contact sharedInstance] setColorsEnabled:YES];
        [[DDTTYLogger_Contact sharedInstance] setForegroundColor:[UIColor greenColor] backgroundColor:nil forFlag:DDLogFlagInfo];
        [[DDTTYLogger_Contact sharedInstance] setForegroundColor:[UIColor redColor] backgroundColor:nil forFlag:DDLogFlagError];
        [[DDTTYLogger_Contact sharedInstance] setForegroundColor:[UIColor orangeColor] backgroundColor:nil forFlag:DDLogFlagWarning];
        [[DDTTYLogger_Contact sharedInstance] setForegroundColor:[UIColor lightGrayColor] backgroundColor:nil forFlag:DDLogFlagVerbose];
    });
    return share;
}
...
//#warning all newly created API (v2) will use PUT method and param in header (@Daniel)
    ...
    if([[paramDic objectForKey:kAPI_REQUEST_KIND] isEqualToString:@"Download"]){
        if(![[paramDic objectForKey:kAPI_REQUEST_METHOD] isKindOfClass:[NSString class]] &&
           ![[paramDic objectForKey:kAPI_REQUEST_METHOD] isEqualToString:@"PUT"]){
           DDLogError(@"%s: Invalid API_REQUEST_METHOD value. Accept 'PUT' Only.",__PRETTY_FUNCTION__);
            return;
        }
    }
    
    id successBlock = ^(AFHTTPRequestOperation_Contact *operation, id responseObject) {
    ...
    __block BOOL didCallCheckMaintenace = FALSE;
    
    id failBlock = ^(AFHTTPRequestOperation_Contact *operation, NSError *error) {
        ...
        //call API check maintance from server
        NSError *firstError = error;
        if (!didCallCheckMaintenace){
            NSDictionary *requestDic = @{
                                        API: API_CHECK_SERVER_MAINTERNACE,
                                       API_VERSION: API_CHECK_SERVER_MAINTERNACE_VERSION,
                                       kAPI_REQUEST_METHOD: @"POST",
                                       kAPI_REQUEST_KIND: @"NORMAL"
                                       };
            
            [self requestService:requestDic tenantServer:NO callback:^(BOOL success, NSString *message, NSDictionary *response, NSError *error) {
                didCallCheckMaintenace = YES;
                if (success) {
                    
                    NSString *mainternaceMessage = [response objectForKey:kMAINTENANCE_MSG];
                    NSMutableDictionary* responseDic = [NSMutableDictionary new];
                    [responseDic setObject:[NSNumber numberWithInt:ERROR_STATUS_CODE_SERVER_MAINTERNACE] forKey:RESPONSE_STATUS_CODE];
                    [responseDic setObject:[NSNumber numberWithBool:NO] forKey:RESPONSE_SUCCESS];
                    [responseDic setObject:[response objectForKey:kMAINTENANCE_MSG] forKey:RESPONSE_STATUS_MSG];
                    DDLogInfo(@"%s: SUCCESS CALL API CHECK SERVER MAINTERNACE", __PRETTY_FUNCTION__);
                    requestCompletionCallBack(NO, mainternaceMessage, responseDic, nil);
                }else{
                    DDLogError(@"%s: FAILED CALL API CHECK SERVER MAINTERNACE", __PRETTY_FUNCTION__);
                    requestCompletionCallBack(NO, @"ailed to call server", nil, firstError);
                }
            }];
        }
    ...
    id successDownloadBlock = ^(AFHTTPRequestOperation_Contact *operation, id responseObject){
        NSMutableDictionary* responseDownloadDic = [NSMutableDictionary new];
        [responseDownloadDic setObject:[operation responseData] forKey:kRESPONSE_DATA];
        requestCompletionCallBack(YES, @"Download file success.", responseDownloadDic, nil);
    };
        ...
        if ([[paramDic objectForKey:kAPI_REQUEST_KIND] isEqualToString:@"Upload"])
        {
            ...
            manager.responseSerializer = [AFJSONResponseSerializer_Contact serializer];
            ...
            AFHTTPRequestOperation_Contact *op = [manager POST:serverURL parameters:parametersURL constructingBodyWithBlock:uploadBlock success:successBlock failure:failBlock];
        ...
        else if ([[paramDic objectForKey:kAPI_REQUEST_KIND] isEqualToString:@"Download"])
        {
            ...
            DDLogWarn(@"Parameters for request API Download: %@", parametersURL);
            
            AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
            manager.responseSerializer = [AFJSONResponseSerializer_Contact serializer];
            manager.responseSerializer.acceptableContentTypes = [[NSSet alloc] initWithObjects:@"text/html",@"image/tiff", @"image/jpeg", @"image/gif", @"image/png", @"image/ico", @"image/x-icon", @"image/bmp", @"image/png", @"image/x-bmp", @"image/x-xbitmap", @"image/x-win-bitmap",@"application/octet-stream",@"audio/mp4",@"audio/mp3",@"audio/mov",@"audio/m4v",@"video/mp4",@"video/mov",@"video/m4v",@"audio/mpeg",@"video/mpeg", nil];
            ...
            [manager PUT:serverURL parameters:parametersURL success:successDownloadBlock failure:failBlock];
