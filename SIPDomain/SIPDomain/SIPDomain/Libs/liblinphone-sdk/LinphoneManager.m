...
//    currentMessage = [NSString stringWithFormat:@"%s", message];
//    //1) I called friend A > Friend A declined my call
//    //Decline
//    if ([currentMessage isEqualToString:@"Call declined."]) {
//        //Insert Cancelled call
//        NSString *jid = [VOIPManager share].receive_jid;
//        [[VOIPManager share] addCallLog:jid Image:@"c_bb_call_canceled" Message:@"No Answer" isMyMessage:YES];
//    }
//    
//    if ([currentMessage isEqualToString:@"Busy here"]) {
//        //Insert Cancelled call
//        NSString *jid = [VOIPManager share].receive_jid;
//        [[VOIPManager share] addCallLog:jid Image:@"c_bb_call_canceled" Message:@"No Answer" isMyMessage:YES];
//    }   
...
//    if (state == LinphoneCallIncomingReceived) {
//        if (![VOIPManager share].isCallEnd) {
//            [[AppDelegate share] switchToIncomingCall];
//        } else {
...
//	}
//    if ((state == LinphoneCallEnd) || (state == LinphoneCallError)) {
//        [VOIPManager share].isOnCall = NO;
//        [BWStatusBarOverlay dismiss];
//        [[VoiceCallView share].view removeFromSuperview];
//        [[IncomingCallView share].view removeFromSuperview];        
//	}    
    /////////////////////////////////////
}
...
- (void)createLinphoneCore {
	...
	mIterateTimer = [NSTimer scheduledTimerWithTimeInterval:0.02
												   ...
												   userInfo:nil
													repeats:YES];
    //Parker add NSRunLoop for avoid automatically increase memory of QuartCore
    NSRunLoop *runner = [NSRunLoop currentRunLoop];
    [runner addTimer:mIterateTimer forMode: NSDefaultRunLoopMode];
}
...
//    if (bluetoothAvailable) {
//        /*
//         - (BOOL)setCategory:(NSString *)category withOptions:(AVAudioSessionCategoryOptions)options error:(NSError **)outError with category as AVAudioSessionCategoryPlayAndRecord or AVAudioSessionCategoryRecord and options as AVAudioSessionCategoryOptionAllowBluetooth
//         */
//        [session setCategory:AVAudioSessionCategoryAudioProcessing withOptions:AVAudioSessionCategoryOptionAllowBluetooth error:&error];
//    }
...
//    //Parker add NSRunLoop for avoid automatically increase memory of QuartCore
//    NSRunLoop *runner = [NSRunLoop currentRunLoop];
//    [runner addTimer:callSecurityTimer forMode: NSDefaultRunLoopMode];
}
