...
@interface DDLoggerNode_XMPP : NSObject {
@public 
    id <DDLogger> logger;   
...
+ (DDLoggerNode_XMPP *)nodeWithLogger:(id <DDLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue logLevel:(int)logLevel;

@end


@interface DDLog_XMPP (PrivateAPI)

+ (void)lt_addLogger:(id <DDLogger>)logger logLevel:(int)logLevel;
+ (void)lt_removeLogger:(id <DDLogger>)logger;
+ (void)lt_removeAllLoggers;
+ (NSArray *)lt_allLoggers;
+ (void)lt_log:(DDLogMessage_XMPP *)logMessage;
+ (void)lt_flush;
...
+ (void)queueLogMessage:(DDLogMessage_XMPP *)logMessage asynchronously:(BOOL)asyncFlag
{
     ...
     format:(NSString *)format, ...
{
    ...
    if (format)
    {
        ...
        DDLogMessage_XMPP *logMessage = [[DDLogMessage_XMPP alloc] initWithLogMsg:logMsg
                                                                  level:level
                                                                   flag:flag
                                                                context:context
                                                                   file:file
...
+ (void)lt_addLogger:(id <DDLogger>)logger logLevel:(int)logLevel
{
    ...
    DDLoggerNode_XMPP *loggerNode = [DDLoggerNode_XMPP nodeWithLogger:logger loggerQueue:loggerQueue logLevel:logLevel];
...
+ (void)lt_removeLogger:(id <DDLogger>)logger
{
    ...
    DDLoggerNode_XMPP *loggerNode = nil;
    
    for (DDLoggerNode_XMPP *node in loggers)
    {
...
+ (void)lt_removeAllLoggers
{
    ...
    for (DDLoggerNode_XMPP *loggerNode in loggers)
    {
...
@implementation DDLoggerNode_XMPP
...
+ (DDLoggerNode_XMPP *)nodeWithLogger:(id <DDLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue logLevel:(int)logLevel
{
    return [[DDLoggerNode_XMPP alloc] initWithLogger:logger loggerQueue:loggerQueue logLevel:logLevel];
}
...
@implementation DDLogMessage_XMPP
...
- (id)copyWithZone:(NSZone *)zone {
    DDLogMessage_XMPP *newMessage = [[DDLogMessage_XMPP alloc] init];
...
@implementation DDAbstractLogger_XMPP
