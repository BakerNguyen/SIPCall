...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(_DONE, nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
    ...
    [UIView animateWithDuration:animationDuration animations:^(void) {
        ...
        for(UIButton* button in self.view.subviews){
            if ([button isKindOfClass:[UIButton class]]) {
                ...
                frame.origin.y = (self.view.frame.size.height - button.frame.size.height)/2;
                button.frame = frame;
            }
        }
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6")) {
                    ...
                    if ([MFMailComposeViewController canSendMail]) {
                        _actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                               cancelButtonTitle:NSLocalizedString(_CANCEL, nil) destructiveButtonTitle:nil
