...
- (void)_setupUseButton{
    ...
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
        ...
        [self.useButton setFrame:CGRectMake(0, 0, 90, 30)];
        [self.useButton setTitle:NSLocalizedString(@"Crop Photo",@"") forState:UIControlStateNormal];
        ...
        [[self.useButton titleLabel] setFont:[UIFont systemFontOfSize:11]];
        [[self.useButton titleLabel] setShadowOffset:CGSizeMake(0, -1)];
        [self.useButton setFrame:CGRectMake(0, 0, 90, 30)];
        [self.useButton setTitle:NSLocalizedString(@"Crop Photo",@"") forState:UIControlStateNormal];
        [self.useButton setTitleShadowColor:[UIColor colorWithRed:0.118 green:0.247 blue:0.455 alpha:1] forState:UIControlStateNormal];
...
- (UIImage *)_toolbarBackgroundImage{
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(self.view.width, 54), YES, 0.0);
